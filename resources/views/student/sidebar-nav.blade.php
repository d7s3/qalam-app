...
@if(\App\Support\RolePages::isEnabled('student', 'student.motivations'))
    <flux:sidebar.item icon="sparkles" :href="route('student.motivations')" :current="request()->routeIs('student.motivations')" wire:navigate>
        مستودع الشواهد
    </flux:sidebar.item>
@endif
    @if(\App\Support\RolePages::isEnabled('student', 'student.my-day'))
        <flux:sidebar.item icon="sun" :href="route('student.my-day')" :current="request()->routeIs('student.my-day')" wire:navigate>
            {{ __('يومي') }}
        </flux:sidebar.item>
    @endif
    @if(\App\Support\RolePages::isEnabled('student', 'student.self-program'))
        <flux:sidebar.item icon="squares-2x2" :href="route('student.self-program')" :current="request()->routeIs('student.self-program')" wire:navigate>
            {{ __('البرنامج الذاتي') }}
        </flux:sidebar.item>
    @endif
