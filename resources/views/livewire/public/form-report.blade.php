<div class="w-full max-w-7xl mx-auto space-y-6 px-2 md:px-6" x-data="{ openDrawer: false }">
    ...
    <!-- Header Block -->
    <div class="bg-white rounded-2xl border border-zinc-200 p-6 md:p-8 shadow-xs relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-sm md:text-base text-zinc-500 mt-2">{{ $form->description ?: 'تقرير استعراض وتحليل البيانات المستلمة.' }}</p>
            </div>
            
            <div class="flex flex-wrap gap-2 text-xs">
                @if(!empty($groupBy))
                    <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full font-medium bg-indigo-50 text-indigo-700 border border-indigo-150">
                        تجميع رئيسي: 
                        {{ collect($form->fields)->firstWhere('id', $groupBy)['label'] ?? '' }}
                    </span>
                @endif
                @if(!empty($subGroupBy))
                    <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full font-medium bg-emerald-50 text-emerald-700 border border-emerald-150">
                        تجميع فرعي: 
                ...
                @if($activeFiltersCount > 0)
                    <span class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full font-medium bg-amber-50 text-amber-700 border border-amber-150">
                        الفلاتر النشطة: {{ $activeFiltersCount }}
                    </span>
                @endif
                <button type="button" wire:click="exportCsv" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full font-bold bg-emerald-600 text-white hover:bg-emerald-700 transition-colors">
            ...
            <div class="divide-y divide-zinc-200">
                @forelse($groupedData as $primaryKey => $group)
                    <div class="p-4 md:p-6 space-y-4">
                        ...
                        <div class="flex items-center gap-2 border-b border-zinc-150 pb-3">
                            <span class="w-3 h-6 rounded-xs" style="background-color: {{ $form->color }}"></span>
                            <h2 class="text-lg font-bold text-zinc-800">
                                {{ collect($form->fields)->firstWhere('id', $groupBy)['label'] ?? '' }}: 
                                <span class="text-zinc-900 underline decoration-2" style="text-decoration-color: {{ $form->color }}">{{ $primaryKey }}</span>
                            </h2>
                            <span class="text-xs px-2 py-0.5 rounded-full bg-zinc-100 text-zinc-500 font-medium">
                        ...
                        @if($hasSubGrouping)
                            <div class="space-y-6 ps-4 border-r-2 border-zinc-100">
                                @foreach($group as $secondaryKey => $subResponses)
                                    <div class="space-y-3">
                                        <!-- Secondary Group Header -->
                                        <div class="flex items-center gap-2">
                                            <span class="w-2 h-4 rounded-xs bg-emerald-500"></span>
                                            <h3 class="text-sm font-bold text-zinc-700">
                                                {{ collect($form->fields)->firstWhere('id', $subGroupBy)['label'] ?? '' }}: 
                                                <span class="text-zinc-900 font-extrabold">{{ $secondaryKey }}</span>
                                            </h3>
                                            <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-semibold">
            ...
            <div class="absolute inset-0 overflow-hidden">
                <div class="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-0">
                    ...
                    <div x-show="openDrawer" 
                        ...
                        <div class="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-2xl border-l border-zinc-200">
                            <!-- Drawer Header -->
                            <div class="px-4 sm:px-6 border-b border-zinc-150 pb-4 flex items-center justify-between">
                                <h2 class="text-lg font-bold text-zinc-900" id="slide-over-title">خيارات العرض والتصفية</h2>
                            ...
                            <div class="relative mt-6 flex-1 px-4 sm:px-6 space-y-6">
                                ...
                                <div class="border border-zinc-200 rounded-xl overflow-hidden" x-data="{ open: true }">
                                    <button @click="open = !open" class="w-full flex items-center justify-between px-4 py-3 bg-zinc-50 hover:bg-zinc-100/50 transition-colors">
                                        <span class="text-sm font-bold text-zinc-800 flex items-center gap-2">
                                    ...
                                    <div x-show="open" x-collapse class="p-4 bg-white space-y-4 border-t border-zinc-200">
                                        <!-- Primary Grouping -->
                                        <div class="space-y-1.5">
                                            <label class="text-xs font-semibold text-zinc-500">التجميع الرئيسي (حسب الحقل)</label>
                                            <select wire:model.live="groupBy" class="block w-full rounded-lg border border-zinc-200 bg-transparent px-3 py-2 text-sm text-zinc-800">
                                        ...
                                        @if(!empty($groupBy))
                                            <div class="space-y-1.5" x-transition>
                                                ...
                                                <select wire:model.live="subGroupBy" class="block w-full rounded-lg border border-zinc-200 bg-transparent px-3 py-2 text-sm text-zinc-800">
                                    ...
                                    <div x-show="open" x-collapse class="p-4 bg-white space-y-4 border-t border-zinc-200 divide-y divide-zinc-150">
                                        @foreach($form->fields as $field)
                                            <div wire:key="filter-field-{{ $field['id'] }}" class="pt-3 first:pt-0 space-y-2">
                                                ...
                                                @if($field['type'] === 'text')
                                                    <!-- Text Input Filter -->
                                                    <input type="text" wire:model.live.debounce.300ms="filters.{{ $field['id'] }}" placeholder="ابحث في الإجابات..." class="block w-full rounded-lg border border-zinc-200 bg-transparent px-3 py-2 text-sm text-zinc-800" />
                                                ...
                                                @elseif($field['type'] === 'date')
                                                    ...
                                                    <input type="text" wire:model.live.debounce.300ms="filters.{{ $field['id'] }}" placeholder="مثال: YYYY-MM-DD..." class="block w-full rounded-lg border border-zinc-200 bg-transparent px-3 py-2 text-sm text-zinc-800" />
                                                @endif
                                            </div>
                                        @endforeach
                                    </div>
